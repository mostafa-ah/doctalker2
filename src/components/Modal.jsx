...
const ModalBox = () => {
  ...
  const navigation = {
    ...
    profile: [
      ...
      {
        name: "Email",
        value: "peter@gmail.competer@gmail.competer@gmail.competer@gmail.competer@gmail.competer@gmail.competer@gmail.competer@gmail.competer@gmail.competer@gmail.com",
      },
    ],
  }
  return (
    <div className="absolute left-1/2 top-1/2 z-20 flex h-[300px] w-[600px] -translate-x-1/2 -translate-y-1/2  transform flex-col gap-3 rounded bg-muted ">
      ...
      <div className="grid h-full w-full grid-cols-3 text-sm">
        <div className="col-span-1 flex flex-col gap-2 px-3 font-bold">
          ...
          <button onClick={()=>setActiveTap("profile")} className={cn("w-full rounded p-1 px-2 hover:bg-foreground/10", {
        ...
        <div className="col-span-2 flex flex-col px-2 pr-6">
          {navigation[activeTap].map((item,index) => (
            <div className={cn("flex gap-12 w-full items-center justify-between border-b py-4 dark:border-foreground/20",{
                "border-none": index === navigation[activeTap].length-1
            })}>
              <h3>{item.name}</h3>
              <p className="overflow-hidden break-all whitespace-nowrap text-ellipsis">{item.value}</p>
            </div>
          ))}
        </div>
      </div>
